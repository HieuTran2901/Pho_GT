...
export function useOrderSectionState(sectionRef, { cartItems = [], onClearCart, onToast } = {}) {
  ...
  // Realtime Polling for Webhook confirmation (3s / poll, up to 15 mins QR lifespan)
  useEffect(() => {
    ...
    // [Issue #6] Tự động đối soát ngay khi tab/app quay lại foreground hoặc focus
    const handleWakeup = () => {
    ...
    // [Issue #5] Nhận thông báo xác nhận tức thì từ tab khác đang mở cùng phiên VietQR
  ...
  }, [step, paymentData?.paymentCode, paymentData?.status, isVietQrConfirmed, handleVerifyPayment, onClearCart]);
